...
class UnityEnv(Environment):
    ...
    def check_compatibility(env):
        if env.number_brains > 1:
            logging.warning("Tensorforce currently doesn't support multiple brains, only default brain {} will be trained".format(env.brain_names[0]))
        env_info = UnityEnv.get_env_info_by_brain(env, env.reset(), 0)
        n_agents = env_info.vector_observations.shape[0]
        if n_agents > 1:
            raise TensorForceError("The provided unity environment has {} agents. Tensorforce currently doesn't support getting observations from multiple agents".format(n_agents))
    @staticmethod
    ...
    def action_from_brain(brain):
        if brain.vector_action_space_type=="discrete":
            ...
            return dict(shape=(brain.vector_action_space_size[0],), type='float', min_value=-5, max_value=5)
